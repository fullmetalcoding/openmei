...
namespace mei {
    ...
    // Not inline: the definition lives in seeing.cpp. Declaring it inline here
    // while defining it out of line is ill-formed -- an inline function must be
    // defined in every translation unit that uses it. MSVC tolerated it; GCC did
    // not, which is how it surfaced only on the Linux runner.
