...
public:
    ...
    // Frame coordinates -> screen coordinates, for drawing overlays on top of
    // the image. Accounts for aspect-fit scaling, centring, and the half-size
    // texture produced when Bayer frames are summed for display. False before
    // anything has been drawn this frame.
    bool  mapFrameToScreen(double fx, double fy, ImVec2& out) const;
    float screenPerFramePx() const { return lastScale_ * texPerFrame_; }
    bool  hasDrawn() const { return drawn_; }
...
private:
    ...
    ImVec2 lastOrigin_{ 0.0f, 0.0f };
    float  lastScale_   = 1.0f;   // screen px per texture px
    float  texPerFrame_ = 1.0f;   // texture px per FRAME px (0.5 when summed)
    bool   drawn_ = false;
