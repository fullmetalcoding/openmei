...
namespace mei::ui {
	...
	void SettingsDialog::drawReporting(DimmConfig& cfg) {
		...
		static constexpr CoefficientModel models[] = {
				CoefficientModel::SarazinRoddier,
				CoefficientModel::SarazinRoddierMini,
				CoefficientModel::YuSasiela,
				CoefficientModel::Manual
		};

		int selected = 0;
		for (int i = 0; i < 4; ++i)
			if (models[i] == r.coefficients)
				selected = i;

		ImGui::SeparatorText("Response coefficients");

		
		if (ImGui::Combo("Model", &selected,
			"Sarazin & Roddier (0.179)\0"
			"Sarazin & Roddier corrected (0.182)\0"
			"Yu/Sasiela\0"
			"Manual K\0"))
			r.coefficients = models[selected];
