...
namespace mei {
    ...
    // Optional interface for sources with a finite, seekable extent. A live camera
    // has no position and no end; a file has both, and the UI needs to know which
    // it is holding rather than inferring it from the backend id.
    class IPlayback {
    public:
        ...
        // 1.0 replays at the capture's own cadence; 0 means as fast as the machine
        // manages, which is what reprocessing a night wants.
    ...
    public:
        ...
        // A finite source that has been exhausted. nextFrame() returning false
        // means "nothing right now" -- for a live camera that is a timeout and more
        // frames may follow, but for a file it can mean there are none left. The
        // consumer needs to tell those apart to close out a burst rather than wait
        // for data that will never arrive. Live backends never end.
        virtual bool endOfStream() const { return false; }

        // True once no further frames will ever arrive. Distinct from nextFrame()
        // returning false, which means "nothing yet, try again" -- a consumer that
        // conflated the two would spin forever at the end of a file.
        virtual bool finished() const { return false; }

        // Non-null for finite, seekable sources. Live cameras return nullptr.
        virtual IPlayback* playback() { return nullptr; }
    };
    ...
    // The connect dialog sets this from a file picker; the backend then presents it
    // as an available "camera". Replay is a frame source like any other, which is
    // what lets a recorded capture run the entire measurement chain unmodified.
