...
namespace {
// Small ring of recent ground truth, written by the generator thread and read
// by analysis. Sized generously relative to any plausible pipeline latency.
constexpr size_t kTruthRing = 4096;
std::mutex                    g_truthMutex;
std::vector<SyntheticTruth>   g_truth(kTruthRing);
}

bool syntheticTruthFor(uint64_t sequence, SyntheticTruth& out) {
    std::lock_guard<std::mutex> lk(g_truthMutex);
    const SyntheticTruth& t = g_truth[sequence % kTruthRing];
    if (!t.valid || t.sequence != sequence) return false;
    out = t;
    return true;
}
...
private:
    ...
    void render(Frame& f) {
        ...
        // Flux-weighted accumulators: the true centroid of an integrated frame
        // is the flux-weighted mean of the instantaneous positions, not their
        // arithmetic mean. Scintillation makes those differ.
        double tAx = 0, tAy = 0, tBx = 0, tBy = 0;
        double tFa = 0, tFb = 0, tDl = 0, tDt = 0;

        for (int s = 0; s < nSub; ++s) {
            if (p.staticMode) {
                diffL_ = diffT_ = 0.0;
                comX_ = p.staticOffsetX;
                comY_ = p.staticOffsetY;
            } else {
            ...
            tAx += ax * fA; tAy += ay * fA; tFa += fA;
            tBx += bx * fB; tBy += by * fB; tFb += fB;
            tDl += diffL_ * fA; tDt += diffT_ * fA;

            if (p.injectFieldStar)
                addSpot(W * 0.25, H * 0.25, fA * 0.15, sigma, sigma, ux, uy);
        }

        // Publish ground truth before the detector stage: this is the noiseless
        // position the image was formed around, so any difference a centroider
        // reports against it is the centroider's own error.
        {
            SyntheticTruth t;
            t.sequence = sequence_;
            if (tFa > 0.0) { t.ax = tAx / tFa; t.ay = tAy / tFa;
                             t.diffLongPx = tDl / tFa; t.diffTranPx = tDt / tFa; }
            if (tFb > 0.0) { t.bx = tBx / tFb; t.by = tBy / tFb; }
            t.fluxA = tFa;
            t.fluxB = tFb;
            t.valid = true;
            std::lock_guard<std::mutex> lk(g_truthMutex);
            g_truth[t.sequence % kTruthRing] = t;
        }
        ...
        for (int i = 0; i < W * H; ++i) {
            ...
            double e;
            if (p.noiseFree) {
                // Tier 0: the only remaining error source is the centroider.
                e = std::min(sig + sky, p.fullWellE);
            } else if (sig <= 0.0) {
