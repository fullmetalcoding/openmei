...
namespace mei {
    ...
    // The SDK uses wide strings for device names and ids on Windows and narrow ones
    // everywhere else -- ToupcamDeviceV2 is declared twice in toupcam.h behind an
    // _WIN32 test. Anything touching those fields has to follow suit.
#if defined(_WIN32)
    using ToupChar = wchar_t;
#else
    using ToupChar = char;
#endif
...
#define MEI_TOUP_SYMBOLS(X)                                                                 \
    ...
    X(HToupcam,    Open,        (const ToupChar*),                                     true)\
