...
uint32_t clientTxn(const httplib::Request& req) {
    if (const std::string* p = findParam(req, "ClientTransactionID")) {
        try { return static_cast<uint32_t>(std::stoul(*p)); } catch (...) {}
    }
    return 0;
}
...
std::string AlpacaServer::generateUniqueId() {
    std::random_device rd;
    std::mt19937_64 g(rd() ^ uint64_t(nowMs()));
...
bool AlpacaServer::start(const AlpacaConfig& c, std::string& err) {
    ...
    cfg_ = c;
    if (cfg_.uniqueId.empty()) cfg_.uniqueId = generateUniqueId();
