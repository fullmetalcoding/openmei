...
namespace mei {
    ...
    struct MeiApp {
        ...
        // Named distinctly from the LogEntry deque above -- both are "history" in
        // plain English, and the collision is not obvious at the declaration.
        SeeingHistory      seeingHistory;
...
namespace mei::ui {
    ...
    static void MenuBar(MeiApp& app) {
        ...
        if (ImGui::BeginMenu("Settings")) {
            if (ImGui::MenuItem("General...", "Ctrl+,")) {
                app.settings.open();
            }
#ifdef OPENMEI_ALPACA
            ImGui::Separator();
...
#endif
            ImGui::Separator();
            if (ImGui::MenuItem("Native REST...", nullptr, nullptr)) {
                app.setStatus(Severity::Info, "Native REST server not implemented yet.");
        ...
        if (ImGui::BeginMenu("View")) {
            ...
            if (ImGui::MenuItem("Differential Waterfall")) {
                app.setStatus(Severity::Info, "Differential Waterfall not implemented yet.");
            }
            ImGui::MenuItem("Seeing History Plot", nullptr, &app.showHistoryPanel);
            ImGui::EndMenu();
        }
    ...
    while (app.running) {
        ...
        mei::ui::SyntheticPanel(app);
        {
            ...
            if (hs.haveResult) {
                ...
                const double nowS = double(duration_cast<seconds>(
                    system_clock::now().time_since_epoch()).count());
                app.seeingHistory.add(hs.result, nowS);
            }
        }
        mei::ui::SeeingHistoryPanel(&app.showHistoryPanel, app.seeingHistory,
            app.dimm, app.prefs.historyView,
            app.processor.status().state != mei::DimmState::Idle);
