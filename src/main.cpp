    ...
    static void MenuBar(MeiApp& app) {
        ...
        if (ImGui::BeginMenu("Edit")) {
            if (ImGui::MenuItem("Preferences...")) {
                app.setStatus(Severity::Info, "Preferences dialog not implemented yet.");
            }
            ImGui::Separator();
    ...
    static void StatusBar(MeiApp& app) {
        ...
        if (ImGui::BeginViewportSideBar("##StatusBar", vp, ImGuiDir_Down, h, flags)) {
            if (ImGui::BeginMenuBar()) {
                ...
                ImGui::SameLine();
                Lamp("CAL", app.calibrated,
                    "Calibrated: plate scale and mask geometry validated.",
                    "NOT CALIBRATED -- run the calibration wizard before measuring. "
                    "Seeing values cannot be published without a plate scale.");
                ImGui::SameLine();
    ...
    static void CentralArea(MeiApp& app) {
        ...
        if (ImGui::Begin("##Central", nullptr, flags)) {
            ...
            if (!app.stream.running()) {
                ...
                if (ImGui::Button("Stop stream")) app.stream.stop();
            }
            ImGui::SameLine();
